...
router.put("/:courseId", auth, authorize(["admin"]), async (req, res) => {
  ...
  if (!title?.trim()) missingFields.push("title");
  if (!description?.trim()) missingFields.push("description");
  if (!duration || isNaN(duration) || duration <= 0) missingFields.push("duration");
  if (!fee || isNaN(fee) || fee <= 0) missingFields.push("fee");
  if (!requirement?.trim()) missingFields.push("requirement");
  if (!contact?.trim()) missingFields.push("contact");
  if (!subjectCode?.trim()) missingFields.push("subjectCode");
  if (!assignedTo?.trim()) missingFields.push("assignedTo");
...
// Pause/unpause a course (Content Admin only)
router.patch("/:courseId/toggle-pause", auth, authorize(["content_admin"]), async (req, res) => {
  const { courseId } = req.params;
  ...
  try {
    ...
    course.isPaused = !course.isPaused;
    await course.save();

    res.status(200).json({ 
      message: `Course ${course.isPaused ? 'paused' : 'unpaused'} successfully`, 
      course 
    });
  } catch (error) {
    console.error("Error toggling course pause status:", error);
    res.status(500).json({ message: "Error toggling course pause status", error: error.message });
  }
});
